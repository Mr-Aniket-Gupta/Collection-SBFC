...
} from "@/features/reports/components/CategoryCards";
import {
  ShareOptionsModal,
  type ShareOption,
} from "@/features/reports/components/ShareOptionsModal";
...
import {
  printElement,
  shareElementAsImage,
} from "@/features/reports/utils/captureUtils";
import {
  downloadMultiSheetWorkbook,
  shareCsvFile,
  toExportRows,
} from "@/features/reports/utils/excelExport";
import {
  buildMisCardMetrics,
  groupTableRowsFromBundle,
} from "@/features/reports/utils/misCardMetrics";
import {
...
import {
  buildReportsFromBundle,
  fetchReportTableBundle,
} from "@/features/reports/utils/reportDataUtils";
import {
  makeReportRow,
  matchesSearch,
} from "@/features/reports/utils/reportHelpers";
import {
  flattenRows,
  isPlainObject,
  looksLikeJsonString,
...
const CATEGORY_CARDS: CategoryCardConfig[] = [
  {
    ...
    title: "Recovery MIS",
    tableKey: "payments",
    icon: <Wallet className="h-5 w-5 text-[var(--color-navy)]" />,
    accent: "bg-[var(--color-navy)]",
    iconBg: "bg-[var(--color-ice)]",
  },
  {
    id: "payment",
    title: "Payment MIS",
  ...
  {
    id: "bucket",
    title: "Bucket-wise MIS",
    tableKey: "dpd-cases",
    icon: <Target className="h-5 w-5 text-[var(--color-gold)]" />,
    accent: "bg-[var(--color-gold)]",
    iconBg: "bg-[rgba(206,155,1,0.12)]",
  },
  {
    id: "digital",
    title: "Digital Recovery",
    tableKey: "payments",
    icon: <Smartphone className="h-5 w-5 text-[#8D6B19]" />,
    accent: "bg-[#8D6B19]",
    iconBg: "bg-[#FDF9F0]",
  },
  {
    ...
    tableKey: "strategies",
    icon: <Share2 className="h-5 w-5 text-[#5B2C6F]" />,
    accent: "bg-[#5B2C6F]",
    iconBg: "bg-[#F4ECF7]",
  },
  {
    ...
    tableKey: "communication_logs",
    icon: <Search className="h-5 w-5 text-[#D35400]" />,
    accent: "bg-[#D35400]",
    iconBg: "bg-[#FDEDEC]",
  },
  {
    ...
    title: "DPD Cases",
    tableKey: "dpd-cases",
    icon: <TrendingUp className="h-5 w-5 text-[#117A65]" />,
    accent: "bg-[#117A65]",
    iconBg: "bg-[#E8F8F5]",
  },
  {
    ...
    tableKey: "bounce-cases",
    icon: <ArrowUpDown className="h-5 w-5 text-[#B03A2E]" />,
    accent: "bg-[#B03A2E]",
    iconBg: "bg-[#FDECEA]",
  },
];
...
export const ReportsPage: React.FC = () => {
  ...
  const {
    ...
    queryFn: () =>
      fetchReportTableBundle(REPORT_LIBRARY_FETCH_LIMIT, [
        "branches",
        "dpd-cases",
        "strategies",
  ...
  const filters = useReportFilters(tableBundle, {
  ...
  const syncBundle = selectedCategory
    ? categoryFilteredBundle
  ...
  const sortedTableRows = useMemo(() => {
    const sortKey =
      effectiveTableColumns.find(
        (c) => shouldFormatDateColumn(c) || c === "status",
      ) ?? effectiveTableColumns[0];
    if (!sortKey) return filteredCurrentTableRows;
    return [...filteredCurrentTableRows].sort((a, b) => {
      const cmp = safeToString(a.source[sortKey]).localeCompare(
        safeToString(b.source[sortKey]),
        undefined,
        { numeric: true },
      );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil((rawTableBundle ? detailedTotal : total) / limit),
  );
  ...
  const channelConversionData = useMemo(
    () => buildChannelConversionData(chartReports),
    [chartReports],
  );
  const bucketWiseTrendData = useMemo(
    () => buildBucketWiseTrendData(chartReports),
    [chartReports],
  );
  const collectionTrendData = useMemo(
    () => buildCollectionTrendData(chartReports),
    [chartReports],
  );
  const recoveryDistributionData = useMemo(
    () => buildRecoveryDistributionData(chartReports),
    [chartReports],
  );
  const paymentVolumeTrend = useMemo(
    () => buildPaymentVolumeTrend(chartReports),
    [chartReports],
  );
  const branchCaseTrend = useMemo(
    () => buildBranchCaseTrend(chartReports),
    [chartReports],
  );
  const communicationFunnel = useMemo(
    () => buildCommunicationFunnel(chartReports),
    [chartReports],
  );
  ...
  const getPrintTarget = (): { element: HTMLElement; title: string } | null => {
    if (activeTab === "Overview" && overviewAnalyticsRef.current) {
      return {
        element: overviewAnalyticsRef.current,
        title: `Reports Overview — ${dateRangeLabel}`,
      };
    }
    if (activeTab === "Detailed Reports" && detailedReportsRef.current) {
      return {
        element: detailedReportsRef.current,
        title: `Raw DCSP Tables — ${prettyTitle(activeTable)}`,
      };
    }
    return null;
  };
      ...
      try {
        ...
        toast.error(
          err instanceof Error
            ? err.message
            : "Unable to export detailed report",
        );
      }
      return;
    }
  ...
  const handleExport = async () => {
    try {
      if (activeTab === "Detailed Reports") {
        ...
        await downloadMultiSheetWorkbook(
          buildExportSheets(),
          `Reports-${dateRangeLabel.replace(/\s+/g, "-")}.xlsx`,
        );
      }
      toast.success("Excel file downloaded");
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : "Unable to export reports",
      );
    }
  };
      ...
      {[
        {
          label: "TOTAL RECORDS",
          value: (
            (syncBundle["dpd-cases"]?.length || 0) +
            (syncBundle["bounce-cases"]?.length || 0)
          ).toLocaleString("en-IN"),
          diff: "Total Cases in Bundle",
          dir: "up" as const,
          icon: <Wallet className="h-5 w-5 text-[var(--color-navy)]" />,
        },
        {
          label: "TOTAL BRANCH",
          value: filters.branchOptions.length.toLocaleString("en-IN"),
          diff: "Available Branches",
          dir: "up" as const,
          icon: <Smartphone className="h-5 w-5 text-[var(--color-navy)]" />,
        },
        {
          label: "TOTAL ZONE",
          value: filters.zoneOptions.length.toLocaleString("en-IN"),
          diff: "Available Zones",
          dir: "up" as const,
          icon: <Target className="h-5 w-5 text-[var(--color-navy)]" />,
        },
        {
          label: "ACTIVE CASES",
          value: (
            (syncBundle["dpd-cases"] || []).filter(
              (c: any) =>
                safeToString(c.loan_status).toLowerCase() === "active",
            ).length +
            (syncBundle["bounce-cases"] || []).filter(
              (c: any) =>
                safeToString(c.status).toLowerCase() === "pending_strategy" ||
                safeToString(c.status).toLowerCase() === "pending",
            ).length
          ).toLocaleString("en-IN"),
          diff: "Cases with active status",
          dir: "up" as const,
          icon: <CheckCircle className="h-5 w-5 text-[var(--color-navy)]" />,
        },
      ].map((kpi) => (
        <div
          key={kpi.label}
          className="reports-kpi-card flex flex-col relative"
        >
          <div className="mb-2 flex items-start justify-between">
            <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--color-ink-muted)]">
              {kpi.label}
            </span>
            <div className="rounded-lg bg-[var(--color-ice)] p-1.5">
              {kpi.icon}
            </div>
          </div>
          <span className="mb-3 text-2xl font-extrabold text-[var(--color-navy)]">
            {kpi.value}
          </span>
  ...
  const renderTabsAndActions = () => (
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
      ...
      ${activeTab === "Overview" ? "text-[var(--color-navy)]" : "text-gray-500"}
    `}
        >
          Overview
        </button>
    ...
    `}
      ...
      <div className="no-print flex items-center gap-3">
        {activeTab !== "Detailed Reports" && (
          <>
            <button
              type="button"
              onClick={handlePrint}
              className="flex items-center gap-2 rounded-lg border border-[rgba(5,0,88,0.08)] bg-white px-3 py-2 text-[13px] font-bold text-[var(--color-navy)] shadow-sm hover:bg-gray-50"
            >
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-lg bg-[var(--color-gold)] px-4 py-2 text-[13px] font-bold text-[var(--color-navy)] shadow-sm hover:brightness-105"
        >
    ...
    <div className="reports-page -m-6 space-y-6 p-6">
      ...
      <div className="reports-section surface-card rounded-xl p-5 space-y-6">
        ...
        {activeTab === "Overview" && (
          <div ref={overviewAnalyticsRef} className="reports-analytics-area">
            ...
            <div className="mt-6 space-y-6">{renderTrends()}</div>
          </div>
        )}

        {activeTab === "Detailed Reports" && (
          <section>
            <h3 className="mb-4 text-[15px] font-bold text-[var(--color-navy)]">
              Raw DCSP Tables
            </h3>

            {/* CASES GROUP */}
            <div className="mb-6">
              <h4 className="mb-2 text-[12px] font-bold uppercase text-[var(--color-ink-muted)]">
                📋 Cases
              </h4>
      ...
      {activeTab === "Detailed Reports" && (
        <div className="space-y-6">
          <div
            ref={detailedReportsRef}
            className="reports-section surface-card rounded-xl p-5 reports-analytics-area"
          >
            <div className="mb-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="no-print">
                <h3 className="text-[15px] font-bold text-[var(--color-navy)]">
                  Table Data
                </h3>
                <p className="text-[12px] text-[var(--color-ink-muted)]">
                  Filtered rows from selected DCSP table
                </p>
              </div>
              <div className="no-print flex flex-col gap-3 md:flex-row">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="min-w-[240px] rounded-xl border border-[rgba(5,0,88,0.12)] bg-white py-2 pl-9 pr-3 text-sm focus:border-[var(--color-gold)] focus:outline-none"
                  />
                </div>
                <div className="relative">
                  ...
                  <select
                    ...
                    className="min-w-[160px] rounded-xl border border-[rgba(5,0,88,0.12)] bg-white py-2 pl-9 pr-3 text-sm focus:border-[var(--color-gold)] focus:outline-none"
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl bg-[var(--color-gold)] px-3 py-2 text-sm font-bold text-[var(--color-navy)] hover:brightness-105"
                >
                  <RotateCw
              ...
              <table className="w-full text-left">
                  ...
                  <tr>
                    {effectiveTableColumns.length > 0 ? (
                      effectiveTableColumns.map((col) => (
                        <th
                          key={col}
                          className="whitespace-nowrap px-4 py-3 text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--color-navy)]"
                        >
                          {col}
                        </th>
                      ))
                    ) : (
                      <th className="px-4 py-3 text-[10px] font-bold uppercase text-[var(--color-navy)]">
                        No columns
                      </th>
                    )}
                  </tr>
                ...
                <tbody className="bg-white">
                  {isLibraryLoading && !rawTableBundle ? (
                    <tr>
                      <td
                        colSpan={Math.max(1, effectiveTableColumns.length)}
                        className="px-4 py-8 text-center text-sm text-[var(--color-ink-muted)]"
                      >
                        <RotateCw className="mx-auto mb-2 h-6 w-6 animate-spin text-[var(--color-gold)]" />
                        Loading...
                      </td>
                    </tr>
                    ...
                    filteredSourceRows.map((row, idx) => (
                      <tr
                        key={idx}
                        className="border-t border-[rgba(5,0,88,0.06)] transition-colors hover:bg-gray-50"
                      >
                        {effectiveTableColumns.map((col) => {
                          const raw = safeToString(row[col]);
                          let rendered = raw;
                          if (shouldFormatDateColumn(col))
                            rendered = tryFormatDate(raw);
                          else if (
                            typeof raw === "string" &&
                            looksLikeJsonString(raw)
                          ) {
                            try {
                              rendered = stringifyCompact(JSON.parse(raw));
                            } catch {
                              /* keep raw */
                            }
                          }
                          return (
                            <td
                              key={col}
                              className="max-w-[260px] truncate px-4 py-3 align-top text-[12px] text-[var(--color-navy)]"
                              title={rendered}
                            >
                              {rendered}
                            </td>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
              <div className="text-xs text-[var(--color-ink-muted)]">
                Showing {detailedTotal === 0 ? 0 : (page - 1) * limit + 1}-
                {Math.min(page * limit, detailedTotal)} of {detailedTotal}
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 text-xs font-medium text-[var(--color-ink-muted)]">
                  {page} / {totalPages}
                </span>
                <button
                  type="button"
