...
export const toExportRows = (rows: DcspTableRow[]): Record<string, string>[] =>
  rows.map((row) =>
    Object.fromEntries(
      Object.entries(row).map(([key, value]) => [key, safeToString(value)]),
    ),
  );
...
export async function downloadMultiSheetWorkbook(
  sheets: ExcelSheet[],
  filename: string,
) {
...
export async function shareExcelWorkbook(
  sheets: ExcelSheet[],
  filename: string,
  shareText?: string,
) {
  ...
  const fallbackShareData = {
    title,
    text: shareText ?? `Sharing ${filename}`,
    url,
  };
...
export async function shareCsvFile(
  ...
  const fallbackShareData = {
    title,
    text: shareText ?? `Sharing ${csvFilename}`,
    url,
  };
