...
export function makeReportRow(
  ...
  return {
    ...
    createdBy,
    date:
      shouldFormatDateColumn("created_at") && rawDate
        ? tryFormatDate(rawDate)
        : rawDate || "-",
    status,
    source: row,
  };
...
export function matchesSearch(
  report: ReportLibraryRow,
  searchTerm: string,
): boolean {
  if (!searchTerm) return true;
  const rowText = Object.values(report.source)
    .map((val) => safeToString(val))
    .join(" ");
  const haystack =
    `${report.name} ${report.id} ${report.category} ${report.createdBy} ${report.status} ${rowText}`.toLowerCase();
