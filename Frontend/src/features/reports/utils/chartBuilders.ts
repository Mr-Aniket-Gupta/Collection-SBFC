...
const monthSortKey = (date: Date): number =>
...
export function normalizeDpdBucket(
  row: Record<string, unknown>,
): (typeof DPD_BUCKETS)[number] {
  ...
  if (bucket.includes("90") || bucket.includes("120") || bucket.includes("+"))
    return "90+";

  return "0-30";
}

const normStatus = (value: unknown): string =>
  safeToString(value).trim().toUpperCase();
...
export function buildChannelConversionData(
  ...
  reports.forEach(({ source }) => {
    ...
    const channel = normalizeCommunicationChannel(
      safeToString(source.channel_name || source.channel),
    );
    if (!channel) return;
...
export function buildBucketWiseTrendData(
  reports: ReportLibraryRow[],
): BucketWiseTrendData[] {
...
export function buildCollectionTrendData(
  reports: ReportLibraryRow[],
): CollectionTrendData[] {
  ...
  return Array.from(summary.entries()).map(([month, values]) => ({
...
export function buildRecoveryDistributionData(
  reports: ReportLibraryRow[],
): RecoveryDistributionData[] {
  ...
  reports.forEach(({ source }) => {
    ...
    const mode =
      safeToString(
        source.payment_mode || source.payment_source || source.payment_status,
      ) || "Unknown";
    const amount = parsePaymentAmount(source);
  ...
  const total =
    Array.from(summary.values()).reduce((sum, val) => sum + val.count, 0) || 1;
...
export function buildPaymentVolumeTrend(
  reports: ReportLibraryRow[],
): PaymentVolumeTrendData[] {
  const summary = new Map<
    string,
    PaymentVolumeTrendData & { sortKey: number }
  >();
  reports.forEach(({ source }) => {
    if (!isPaymentRow(source)) return;
    const date = extractRowDate(source);
...
export function buildBranchCaseTrend(
  reports: ReportLibraryRow[],
): TrendSeriesData[] {
  ...
  reports.forEach(({ source }) => {
    ...
    const branch =
      getBranchName(source) ||
      safeToString(source.zone) ||
      safeToString(source.state) ||
      "Unknown";
    if (safeToString(source.status).toLowerCase().includes("close")) return;
    if (safeToString(source.loan_status).toLowerCase().includes("close"))
      return;
...
export function buildCommunicationFunnel(
  reports: ReportLibraryRow[],
): FunnelStageData[] {
  const stages = [
    ...
    },
    {
      stage: "Read",
      match: (status: string) =>
        status.includes("read") || status.includes("open"),
    },
    {
      stage: "Responded",
      match: (status: string) =>
        status.includes("respond") || status.includes("reply"),
    },
    {
      stage: "Converted",
      match: (status: string) =>
  ...
  const commRows = reports.filter(({ source }) =>
    isCommunicationLogsRow(source),
  );
