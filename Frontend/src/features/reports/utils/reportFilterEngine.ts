...
import { isCommunicationLogsRow, isPaymentRow } from "./rowDetectors";
...
export const CATEGORY_FILTER_CONFIG: Record<string, CategoryFilterConfig> = {
  "Recovery MIS": {
    ...
    primaryPredicate: (row) =>
      isPaymentRow(row) && norm(row.payment_status) === "SUCCESS",
  },
  "Bucket-wise MIS": {
  ...
  "Digital Recovery": {
    primaryTable: "payments",
    primaryPredicate: (row) =>
      isPaymentRow(row) && norm(row.payment_mode) !== "CASH",
  },
  "Payment MIS": {
    primaryTable: "payments",
  ...
  "Bounce Cases": {
    ...
    primaryPredicate: (row) =>
      norm(row.status) === "PENDING_STRATEGY" || norm(row.status) === "PENDING",
  },
  "Communication Reports": {
    primaryTable: "communication_logs",
    primaryPredicate: (row) =>
      isCommunicationLogsRow(row) && norm(row.status) === "DELIVERED",
  },
  "Bounce Analysis": {
    primaryTable: "bounce-cases",
...
const seedIdsFromPrimaryRow = (
  row: DcspTableRow,
  tableKey: ReportTableKey,
  ids: GlobalFilterIds,
) => {
  const getCaseId = (r: any) =>
    id(r.dpd_case_id ?? r.pre_emi_case_id ?? r.bounce_case_id ?? r.case_id);
  switch (tableKey) {
    case "payments":
...
export function resolveGlobalFilterIds(
  ...
  const primaryRows = bundle[config.primaryTable].filter(
    config.primaryPredicate,
  );
  const ids = emptyIds();

  primaryRows.forEach((row) =>
    seedIdsFromPrimaryRow(row, config.primaryTable, ids),
  );
...
export function buildGlobalFilterContext(
  ...
  const primaryMatchCount = bundle[config.primaryTable].filter(
    config.primaryPredicate,
  ).length;
...
const rowMatchesIds = (
  ...
  const caseId = id(
    row.dpd_case_id ?? row.pre_emi_case_id ?? row.bounce_case_id ?? row.case_id,
  );
  const loanNumber = id(row.loan_number);
  const strategyId = id(row.strategy_id);
  ...
  REPORT_TABLE_KEYS.forEach((tableKey) => {
    if (tableKey === "branches") {
      ...
      filtered[tableKey] = bundle[tableKey].filter((row) =>
        rowMatchesIds(row, tableKey, ids),
      );
    }
  });
  return filtered;
...
export function applyCategoryGlobalFilter(
  ...
  filtered[config.primaryTable] = bundle[config.primaryTable].filter(
    config.primaryPredicate,
  );
