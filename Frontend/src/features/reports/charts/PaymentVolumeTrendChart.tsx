...
export const PaymentVolumeTrendChart: React.FC<
  PaymentVolumeTrendChartProps
> = ({ data }) => {
  ...
  const averageTicket =
    summary.volume > 0 ? summary.amount / summary.volume : 0;
      ...
      <div className="space-y-4">
            ...
            >
              ...
              <Tooltip
                ...
                formatter={(value, name) => [
                  name === "Amount" ? formatAmount(value as number) : value,
                  name as string,
                ]}
              />
              <Legend
        ...
        <div className="overflow-hidden rounded-xl border border-[rgba(5,0,88,0.08)] bg-white">
          ...
          {data.slice(-4).map((entry) => (
            <div
              key={entry.month}
              className="grid grid-cols-5 gap-2 px-3 py-2 text-[12px] text-[var(--color-navy)] odd:bg-[var(--color-ice)]/30"
            >
