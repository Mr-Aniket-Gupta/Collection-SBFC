...
function xyToLatLon(x: number, y: number): { lat: number; lon: number } {
  return {
    lon:
      INDIA_BOUNDS.minLon +
      ((x - INDIA_MAP_FRAME.left) / INDIA_MAP_FRAME.width) *
        (INDIA_BOUNDS.maxLon - INDIA_BOUNDS.minLon),
    lat:
      INDIA_BOUNDS.maxLat -
      ((y - INDIA_MAP_FRAME.top) / INDIA_MAP_FRAME.height) *
        (INDIA_BOUNDS.maxLat - INDIA_BOUNDS.minLat),
  };
}
...
function categoryColor(hex: HexTile | null): string {
  ...
  if (hex.category === "Low density" || hex.category === "Sparse activity")
    return PALETTE.hexTexture;
  ...
  return raw
    ...
    .map((loc, i) => {
      ...
      return {
        ...
        cardSide:
          loc.cardSide === "left" || loc.cardSide === "right"
            ? loc.cardSide
            : xPct > 60
              ? "left"
              : "right",
...
export default function HexWorldMap({
  ...
  const hexData: HexTile[] = useMemo(() => {
    return hexCenters.map((hc) => {
      ...
      const isSelectedRing = Boolean(
        selectedId && nearest?.id === selectedId && nearestDist < 20,
      );
    ...
    >
      ...
      <div className="absolute right-4 top-4 flex flex-col gap-1.5">
        <button
          ...
          aria-label="Zoom in"
          title="Zoom in"
          className="w-9 h-9 rounded-xl bg-white/95 shadow-md flex items-center justify-center hover:bg-white"
        >
      ...
      <div
        className="absolute right-4 top-[9.5rem] text-[10px] font-medium px-2 py-0.5 rounded-md bg-white/80"
        style={{ color: PALETTE.subtext }}
      >
          ...
          const rows: { icon: LucideIcon; label: string; value: string }[] = [
            {
              icon: MapPinned,
              label: "Nearest hub",
              value: selectedHex.nearest ? selectedHex.nearest.name : "—",
            },
            {
              icon: Users,
              label: "Users",
              value: selectedHex.nearest
                ? formatNumber(selectedHex.nearest.value)
                : "—",
            },
            {
              icon: Ruler,
              label: "Distance",
              value: `${selectedHex.nearestDist} px`,
            },
            {
              icon: Compass,
              label: "Lat / Lon",
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div
                        className="text-[10.5px] font-medium tracking-wide uppercase"
                        style={{ color: PALETTE.subtext }}
                      >
                  ...
                  <button
                    onClick={() => setSelectedHex(null)}
                    className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-black/5 shrink-0 transition-colors"
                    aria-label="Close"
                  >
                ...
                <div
                  className="mt-3 pt-2.5 border-t border-black/[0.06] text-[10px] font-mono tracking-tight"
                  style={{ color: PALETTE.subtext }}
                >
