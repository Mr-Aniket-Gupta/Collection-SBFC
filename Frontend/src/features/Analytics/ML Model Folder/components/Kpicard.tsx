...
export function KPICard({ data }: KPICardProps) {
    return (
        <div className="group relative overflow-hidden rounded-2xl border border-[rgba(5,0,88,0.1)] bg-white p-4 shadow-[0_8px_24px_rgba(5,0,88,0.05)] transition duration-300 hover:-translate-y-0.5 hover:border-[rgba(0,1,130,0.25)]">
            ...
            <p className="mb-2 text-xs font-medium uppercase tracking-[0.12em] text-[var(--color-ink-muted)]">{data.label}</p>
            <div className="flex items-baseline gap-2">
                <p className="text-2xl font-semibold tracking-tight text-[var(--color-navy)]">{data.value}</p>
                {data.trend && (
                    <span className={`text-xs font-semibold ${trendColor[data.trend.direction]}`}>
