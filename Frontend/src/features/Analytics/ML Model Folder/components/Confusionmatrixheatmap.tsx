...
export function ConfusionMatrixHeatmap({ data }: ConfusionMatrixHeatmapProps) {
    ...
    return (
        <div className="relative h-full overflow-x-auto rounded-2xl border border-[rgba(5,0,88,0.1)] bg-white p-5 shadow-[0_8px_24px_rgba(5,0,88,0.05)]">
            ...
            <h3 className="mb-1 text-sm font-semibold tracking-tight text-[var(--color-navy)]">Confusion matrix</h3>
            ...
            <table className="text-xs" style={{ borderCollapse: "collapse" }}>
                <thead>
                    <tr>
                        <th className="p-2"></th>
                        {labels.map((label) => (
                            <th key={label} className="p-2 text-center font-medium text-[var(--color-ink-muted)]">
                                {label}
                            </th>
                        ))}
                    </tr>
