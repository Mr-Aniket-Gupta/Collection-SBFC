...
export const AgentContributionChart: React.FC<AgentContributionChartProps> = ({
  data,
}) => (
  <ChartCard
    title="Top Agent Contributors"
    subtitle="Allocated, resolved, and recovery mix by agent"
    data={data}
  >
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Bar dataKey="resolvedCases" name="Resolved" radius={[6, 6, 0, 0]}>
            {data.map((entry, index) => (
              <Cell
                key={`${entry.agentName}-resolved`}
                fill={
                  ["#3A7D44", "#5AA469", "#1D4ED8", "#7C3AED", "#D97706"][
                    index % 5
                  ]
                }
              />
