  ...
  if (active && payload && payload.length > 0) {
    return (
      <div className="bg-white border border-[rgba(5,0,88,0.12)] rounded-xl px-3 py-2.5 shadow-lg">
        ...
        <p className="text-[16px] font-bold text-[var(--color-navy)]">
  ...
  return (
    ...
    >
      <ResponsiveContainer width="100%" height={350}>
        ...
        >
          <PolarGrid stroke="#D9EAF5" strokeDasharray="4 2" />
          <PolarAngleAxis
            dataKey="metric"
            tick={{
              fontSize: 11,
      ...
      <div className="flex flex-wrap gap-2 mt-4 justify-center">
        {data.map((item) => (
          ...
          >
            <span className="text-[10px] font-semibold text-[var(--color-blue)]">
              {item.metric}
            </span>
