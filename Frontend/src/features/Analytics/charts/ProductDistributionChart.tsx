    ...
    return (
      <div className="bg-white border border-[rgba(5,0,88,0.12)] rounded-xl px-4 py-3 shadow-xl">
        <div className="flex items-center gap-2 mb-1">
          ...
          />
          <p className="text-[12px] font-semibold text-[var(--color-navy)]">
            {payload[0].name}
          </p>
        </div>
        <p className="text-[18px] font-bold text-[var(--color-navy)]">
          {formatCappedPercent(Number(payload[0].value))}
        </p>
      </div>
    );
  }
...
> = ({
  ...
  return (
    <ChartCard className="h-full" title={title} subtitle={subtitle} data={data}>
      <div className="flex flex-col items-center">
        ...
        <div className="relative w-full" style={{ height: 240 }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <p className="text-[28px] font-bold text-[var(--color-navy)]">
              {formatCappedPercent(total)}
            </p>
          ...
          {data.map((item) => (
            <div key={item.name} className="flex items-center gap-2.5">
              ...
              <div className="flex items-center justify-between flex-1 min-w-0">
                <span className="text-[12px] text-[var(--color-ink-muted)] font-medium truncate">
                  {item.name}
                </span>
                <span className="text-[12px] font-bold text-[var(--color-navy)] ml-1">
                  {formatCappedPercent(item.value)}
                </span>
              </div>
            </div>
          ))}
