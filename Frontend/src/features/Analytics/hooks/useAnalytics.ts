...
export function useAnalytics() {
  ...
  const { data, isFetching, refetch, error } = useQuery({
    queryKey: ['analyticsDashboard', filters.dateRange, filters.customFromDate, filters.customToDate, filters.branchFilter, filters.zoneFilter, filters.stateFilter],
    queryFn: async () => {
      const branchForApi = filters.branchFilter ? (branchMapRef.current.get(filters.branchFilter) ?? filters.branchFilter) : undefined
      console.debug('[Analytics] fetching dashboard with branchForApi=', branchForApi, 'branchFilter=', filters.branchFilter, 'zone=', filters.zoneFilter, 'state=', filters.stateFilter)
      const resp = await analyticsService.fetchDashboard(filters.dateRange, filters.customFromDate, filters.customToDate, branchForApi, filters.zoneFilter, filters.stateFilter)
