...
import { getDefaultCustomFromDate, getDefaultCustomToDate, DEFAULT_DATE_RANGE } from '@/Components/dateFilter'
import type { ReportTableBundle } from '@/features/reports/utils/reportFilterEngine'
import { getBranchName, safeToString } from '@/features/reports/utils/tableUtils'

export interface UseReportFiltersOptions {
  storageKeyPrefix?: string
}

export function useReportFilters(bundle: ReportTableBundle | undefined, options?: UseReportFiltersOptions) {
  ...
  const [customFromDate, setCustomFromDate] = useState<string>(() => readStored('CustomFrom', getDefaultCustomFromDate()))
  const [customToDate, setCustomToDate] = useState<string>(() => readStored('CustomTo', getDefaultCustomToDate()))
  const [branchFilter, setBranchFilter] = useState<string>(() => readStored('Branch', ''))
  const [zoneFilter, setZoneFilter] = useState<string>(() => readStored('Zone', ''))
  const [stateFilter, setStateFilter] = useState<string>(() => readStored('State', ''))
  ...
  return {
    ...
    branchOptions, zoneOptions, stateOptions,
    resetFilters
  }
}
