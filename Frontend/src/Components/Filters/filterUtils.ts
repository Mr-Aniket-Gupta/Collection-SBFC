...
import { EMPTY_BUNDLE, REPORT_TABLE_KEYS, type ReportTableBundle } from '@/features/reports/utils/reportFilterEngine'

export const norm = (value: unknown): string => safeToString(value).toString().trim().replace(/\s+/g, ' ').toUpperCase()
...
export function filterBundleByBranchZone(
  ...
  const processCaseRow = (row: DcspTableRow) => {
    ...
    if (allowedBranches.has(branchVal)) {
      const caseId = id(row.case_id ?? row.dpd_case_id ?? row.bounce_case_id ?? row.pre_emi_case_id)
