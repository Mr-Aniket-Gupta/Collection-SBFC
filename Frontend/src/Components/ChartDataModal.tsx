...
export const ChartDataModal: React.FC<ChartDataModalProps> = ({
  isOpen,
  onClose,
  title,
  data,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-6 bg-[rgba(5,0,88,0.4)] backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-white rounded-2xl shadow-2xl w-full h-full max-w-[1600px] flex flex-col overflow-hidden animate-[slideUp_0.3s_ease-out]">
        <div className="flex items-center justify-between p-5 sm:p-6 border-b border-[rgba(5,0,88,0.08)] bg-[var(--color-ice)]">
          <div>
            <h3 className="text-xl sm:text-2xl font-bold text-[var(--color-navy)]">
              {title}
            </h3>
          ...
          <button
            onClick={onClose}
            className="p-2 sm:p-3 rounded-xl hover:bg-[rgba(5,0,88,0.1)] text-[var(--color-navy)] transition-colors"
          >
        ...
        <div className="flex-1 overflow-auto p-4 sm:p-6 bg-white relative">
          {!isRendered ? (
            <div className="flex flex-col items-center justify-center h-full opacity-60">
              ...
              <p className="mt-4 text-sm font-medium text-[var(--color-navy)]">
                Loading data view...
              </p>
            </div>
          ) : data.length === 0 ? (
            <div className="flex items-center justify-center h-full text-[var(--color-ink-muted)]">
              No data available
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-[var(--color-ice)] sticky top-0 shadow-sm z-10">
                  <tr>
                    {columns.map((col) => (
                      <th
                        key={col}
                        className="px-4 py-3 sm:px-6 sm:py-4 text-[11px] sm:text-[12px] uppercase tracking-[0.16em] font-bold text-[var(--color-navy)] whitespace-nowrap border-b border-[rgba(5,0,88,0.08)]"
                      >
                        {col.replace(/_/g, " ")}
                      </th>
                    ))}
                ...
                <tbody>
                  {data.map((row, i) => (
                    <tr
                      key={i}
                      className="border-b border-[rgba(5,0,88,0.06)] hover:bg-[rgba(5,0,88,0.02)] transition-colors"
                    >
                      {columns.map((col) => {
                        const val = row[col];
                        let rendered = "-";
                        ...
                        return (
                          <td
                            key={col}
                            className="px-4 py-3 sm:px-6 sm:py-4 text-[13px] sm:text-[14px] text-[var(--color-navy)] max-w-[200px] sm:max-w-[400px] truncate"
                            title={rendered}
                          >
                            {rendered}
                          </td>
